...
class GraphWaveNet(nn.Module):
    ...
    def __init__(self, num_nodes, supports, dropout=0.3, gcn_bool=True, addaptadj=True, aptinit=None, in_dim=2,
        ...
        self.fc_periodic = nn.Sequential(nn.Linear(12, 96), nn.ReLU(), nn.Linear(96, 256))
        ...
        # self.end_conv_1 = nn.Conv2d(in_channels=256*3, out_channels=end_channels, kernel_size=(1, 1), bias=True)
        self.end_conv_2 = nn.Conv2d(in_channels=end_channels, out_channels=out_dim, kernel_size=(1, 1), bias=True)

        self.receptive_field = receptive_field

    def forward(self, input, hidden_states=None, periodic=None):
        ...
        hidden_states_st = self.fc_his(hidden_states)  # B, N, D
        hidden_states_st = hidden_states_st.transpose(1, 2).unsqueeze(-1)
        skip = skip + hidden_states_st
        fc_periodic_emb = self.fc_periodic(periodic[:, :, :, 0].squeeze(-1).transpose(1, 2)).unsqueeze(-1).transpose(1, 2)
        skip = skip + fc_periodic_emb
